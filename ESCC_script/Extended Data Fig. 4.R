...
theme_niwot <- function(){
  theme(
    legend.key=element_blank(), legend.text = element_text(color="black",size=8), legend.spacing.x=unit(0.1,'cm'),
...
ggsave(
  filename = "Tissue_neighbor.pdf", plot = p, device = "pdf",width = 5, height = 5, dpi = 300 )
...
ggsave(
  filename = "Sample_neighborhood10_cluster_label.pdf",
  plot = final_plot,  device = "pdf",   width = 20, height = 5, dpi = 300）
)
...
ggsave(
  filename = "Sample_neighborhood10_boxplot_format.pdf",plot = p,
  device = "pdf",width = 15,height = 5,dpi = 300）
)
