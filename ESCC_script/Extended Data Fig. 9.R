...
plot_data <- fread('E9_a_top5_prop.csv')

create_boxplot <- function(data, module_name) {
  wilcox_test <- wilcox.test(
    prop_top5 ~ metastasis, 
    data = data,
    exact = FALSE,
    correct = TRUE
  )
  ...
  stats_summary <- data %>%
    ...
    summarise(
      n = n(),
      mean = mean(prop_top5, na.rm = TRUE),
      median = median(prop_top5, na.rm = TRUE),
      sd = sd(prop_top5, na.rm = TRUE),
      se = sd / sqrt(n),
      .groups = "drop"
    )
  
  metastasis_colors <- c("N" = "#87CEEB", "Y" = "#CD5C5C")
  
  p <- ggplot(data, aes(x = metastasis, y = prop_top5, fill = metastasis)) +
    geom_boxplot(
      width = 0.6,
      alpha = 0.7,
      outlier.shape = NA,
    ...
    scale_fill_manual(values = metastasis_colors, name = "Metastasis") +
    scale_color_manual(values = metastasis_colors, guide = "none") +
    labs(
      title = module_name,
      subtitle = sprintf("p = %.3f", wilcox_test$p.value),
      x = NULL,
      y = "Proportion of Top 5% Cells"
    ) +
    ylim(0, max(data$prop_top5, na.rm = TRUE) * 1.1) +
...
plots <- lapply(unique(plot_data$module), function(mod) {
  create_boxplot(plot_data[module == mod], mod)
})
...
final_plot <- combined_plot +
  plot_annotation(
    title = "Proportion of Top 5% Cells Across Modules by Metastasis Status",
...
moran_df_final<- fread('E9_a_moran_df_final.csv')

create_moran_boxplot <- function(data, module_name) {
  ...
  p <- ggplot(data, aes(x = metastasis, y = moran_i, fill = metastasis)) +
    geom_boxplot(
      width = 0.6,
      alpha = 0.7,
      outlier.shape = NA,
    ...
    ylim(
      min(data$moran_i, na.rm = TRUE) * 1.1,
      max(data$moran_i, na.rm = TRUE) * 1.1
    ) +
    theme_minimal(base_size = 10) +
    theme(
      plot.title = element_text(hjust = 0.5, face = "bold", size = 12),
 ...
 ----------------------------------------------
plots <- lapply(unique(moran_df_final$module), function(mod) {
  create_moran_boxplot(
    moran_df_final %>% filter(module == mod),
    gsub("_module_NGMS_score", "", mod)
  )
})
...
final_plot <- combined_plot +
  plot_annotation(
    title = "Moran's I of Module Scores by Metastasis Status",
    caption = "Statistical test: Wilcoxon rank-sum test",
    theme = theme(
      plot.title = element_text(hjust = 0.5, face = "bold", size = 16),
      plot.caption = element_text(hjust = 0.5, size = 10, color = "gray50")
...
df_filtered_iqr_grouped <- ICA_ratio %>% left_join(metastasis,by = "file") %>%
  ...
  filter(ratio >= lower_bound & ratio <= upper_bound) %>% select(-Q1, -Q3, -IQR, -lower_bound, -upper_bound)
