#Figure 6a
library(data.table); library(dplyr); library(ggplot2); library(ggpubr); library(patchwork)

create_boxplot <- function(data, condition_label) {
  wilcox_test <- wilcox.test(
    prop_mimer ~ metastasis,
    data = data,
    exact = FALSE,
    correct = TRUE
  )
    ...
    summarise(
      n = n(),
      mean = mean(prop_mimer, na.rm = TRUE),
      median = median(prop_mimer, na.rm = TRUE),
      sd = sd(prop_mimer, na.rm = TRUE),
      se = sd / sqrt(n),
      .groups = "drop"
    )
  ...
  p <- ggplot(data, aes(x = metastasis, y = prop_mimer, fill = metastasis)) +
    geom_boxplot(
      width = 0.6,
      alpha = 0.7,
      outlier.shape = NA,
    ...
    labs(
      ...
      subtitle = sprintf("Wilcoxon p = %.3f", wilcox_test$p.value),
      x = NULL,
      y = "Proportion of MIMER cells (top5%)"
    ) +
    ylim(0, max(data$prop_mimer, na.rm = TRUE) * 1.1) +
...
# 1.read
plot_data_me <- fread('F6_a_me.csv')
plot_data_me <- fread('F6_a_dev.csv')
# 2. draw ----------------------------------------------------
p_me <- create_boxplot(plot_data_me, "ME")
p_dev <- create_boxplot(plot_data_dev, "development")

# 3. combine ---------------------------------------------------
combined_plot <- wrap_plots(p_me, p_dev, nrow = 1, ncol = 2, guides = "collect") &
  theme(legend.position = "bottom")

final_plot <- combined_plot +
  plot_annotation(
    title = "Proportion of MIMER Cells (top5%) by Metastasis Status",
    subtitle = "Subsetting based on ME or development status (excluding unknown)",
