  ...
  spatial_df <- spatial_df %>%
    mutate(
      temp_label = case_when(
        ...
        peripheral_label > 0 ~ peripheral_label + 1000,
        TLS_cluster < 0 ~ -1,
        TRUE ~ 0
      )
    )
...
tls_cluster_mapping <- fread("./TLS_cluster_mapping.csv") %>% 
  as.data.frame() %>%
  setnames(., old = "TLS_label", new = "TLS_cluster_label")
...
tls_cluster_mapping <- tls_cluster_mapping %>%
  ...
  select(-prefix, -patient_code)

library(dplyr)
library(ggplot2)
library(patchwork)

cluster_percent <- tls_cluster_mapping %>%  count(patient, TLS_cluster) %>% group_by(patient) %>% mutate(
    total = sum(n),  percent = n / total * 100) %>%  ungroup()
...
top_plot <- ggplot(tls_count, aes(x = patient, y = TLS_count)) +  geom_bar(stat = "identity", fill = "grey50", alpha = 0.8) + 
  geom_text(aes(label = TLS_count), vjust = -0.5, size = 4) + ylim(0,14)+
