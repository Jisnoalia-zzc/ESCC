...
library(dplyr)
library(msigdbr)
.libPaths(c("/lustre/home/xhzh/R/special", "/lustre/home/xhzh/R/x86_64-pc-linux-gnu-library/4.0", "/lustre/home/xhzh/sw/R-4.0.5/library"))
library(fgsea)

pdf(file="/lustre/home/xhzh/scSpatial/xiaodu/codes/zxh/TLS/TLS_mfuzz_delnontls.pdf", width=20, height=14)
...
d<-NULL
#tissue<-c("Nor","Hyp","MiD","MoD","SD&CA","ICA","MCA")
#tissue <- c("Non-TLS", "TLS_G1", "TLS_G2", "TLS_G3", "peri_TLS_G1", "peri_TLS_G2", "peri_TLS_G3")
tissue <- c("TLS_G1", "TLS_G2", "TLS_G3", "peri_TLS_G1", "peri_TLS_G2", "peri_TLS_G3")
for(i in tissue){
    #cell<-rownames(p@meta.data[p$development==i,])
...
write.table(gene_cluster, "/lustre/home/xhzh/scSpatial/xiaodu/codes/zxh/TLS/TLS_mfuzz_genelist_delnontls.txt", row.names=TRUE, quote=FALSE, sep="\t") 

all_gsea <- data.frame()
all_gsea1 <- data.frame()
markGenes <- c()
#markGenes <- c("PTK2", "SFRP2", "MDK", "VIM", "MYC", "CXCL14", "TWISTNB", "CXCR4", "MDK", "ACSL1", "BARX2")
...
write.table(all_gsea, "/lustre/home/xhzh/scSpatial/xiaodu/codes/zxh/TLS/TLS_mfuzz_gsva_delnontls.txt", row.names=TRUE, quote=FALSE, sep="\t")
write.table(all_gsea1, "/lustre/home/xhzh/scSpatial/xiaodu/codes/zxh/TLS/TLS_mfuzz_gsvalist_delnontls.txt", row.names=TRUE, quote=FALSE, sep="\t")

cm <- clusterData(exp = d, cluster.method = "mfuzz", cluster.num = n, seed=123)

#enrich <- enrichCluster(object = cm, OrgDb = org.Mm.eg.db, type = "BP", pvalueCutoff = 0.05, topn = 5, seed = 123)
#enrich <- enrichCluster(object = cm, OrgDb = org.Mm.eg.db, type = "BP", organism = "hsa", pvalueCutoff = 0.5, topn = 5, seed = 123)

#markGenes = rownames(d)[sample(1:nrow(d),30,replace = F)]
#markGenes
length(markGenes)
...
#visCluster(object = cm, plot.type = "both", column_names_rot = 45, show_row_dend = F, markGenes = markGenes, markGenes.side = "left", annoTerm.data = all_gsea, line.side = "left", go.col = rep(jjAnno::useMyCol("stallion",n = 12),each = 5), add.bar = T)

#visCluster(object = cm, plot.type = "both", column_names_rot = 45, show_row_dend = F, markGenes = markGenes, markGenes.side = "left", annoTerm.data = all_gsea, line.side = "left", go.col = rep(ggsci::pal_d3()(12),each = 5), add.bar = T)
visCluster(object = cm, plot.type = "both", column_names_rot = 45, show_row_dend = F, markGenes = markGenes, markGenes.side = "left", line.side = "left")
print("ok0")
...
print("ok1")
#pdf(file="/lustre/home/xhzh/scSpatial/xiaodu/codes/zxh/Figure1t.pdf", width=20, height=20, onefile=FALSE)
...
#go.col = rep(jjAnno::useMyCol("stallion",n = 12),each = 5)
#visCluster(object = cm, plot.type = "line", ms.col = c("green","orange","red"))
#visCluster(object = cm, plot.type = "line", ms.col = c("green","orange","red"), add.mline = FALSE)
#visCluster(object = cm, plot.type = "heatmap", show_row_dend = F, column_names_rot = 45)
#visCluster(object = cm, plot.type = "both", column_names_rot = 45)
